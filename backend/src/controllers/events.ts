...
timelineController.get("/recevingcare/:pateintId/:dates", (req, res) => {
      connection.query(`SELECT payload FROM events WHERE care_recipient_id = '${req.params.pateintId}' AND (timestamp between '${req.params.dates}T00:00:00.000Z' and '${req.params.dates}T23:59:59.000Z') ORDER BY timestamp;`, async (err, results) => {
        if (err) {
          return res.send(err)
        } else {
          const parsedData = await results.map( (each: Patient) => JSON.parse(each.payload) )
          // Format Date
